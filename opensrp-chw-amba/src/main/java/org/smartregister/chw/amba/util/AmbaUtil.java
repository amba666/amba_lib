...
public class AmbaUtil {
    ...
    public static void processEvent(AllSharedPreferences allSharedPreferences, Event baseEvent) throws Exception {
        if (baseEvent != null) {
            AmbaJsonFormUtils.tagEvent(allSharedPreferences, baseEvent);
            JSONObject eventJson = new JSONObject(AmbaJsonFormUtils.gson.toJson(baseEvent));
    ...
    public static ECSyncHelper getSyncHelper() {
        return AmbaLibrary.getInstance().getEcSyncHelper();
    }

    public static ClientProcessorForJava getClientProcessorForJava() {
        return AmbaLibrary.getInstance().getClientProcessorForJava();
    }
    ...
    public static boolean launchDialer(final Activity activity, final BaseAmbaCallDialogContract.View callView, final String phoneNumber) {
        if (ContextCompat.checkSelfPermission(activity, Manifest.permission.READ_PHONE_STATE) != PackageManager.PERMISSION_GRANTED) {

            // set a pending call execution request
            if (callView != null) {
                callView.setPendingCallRequest(() -> AmbaUtil.launchDialer(activity, callView, phoneNumber));
            }
    ...
    public static void saveFormEvent(final String jsonString) throws Exception {
        AllSharedPreferences allSharedPreferences = AmbaLibrary.getInstance().context().allSharedPreferences();
        Event baseEvent = AmbaJsonFormUtils.processJsonForm(allSharedPreferences, jsonString);
        AmbaUtil.processEvent(allSharedPreferences, baseEvent);
    }
    ...
    public static class CloseAmbaMemberFromRegister extends AsyncTask<Void, Void, Void> {
        private final String baseEntityId;

        public CloseAmbaMemberFromRegister(String baseEntityId) {
            this.baseEntityId = baseEntityId;
        }

        @Override
        protected Void doInBackground(Void... voids) {
            AmbaDao.closeAmbaMemberFromRegister(baseEntityId);
            return null;
        }
