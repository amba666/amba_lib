package org.smartregister.chw.amba.dao;
...
public class AmbaDao extends AbstractDao {

    public static Date getAmbaTestDate(String baseEntityID) {
        String sql = "select amba_test_date from ec_amba_confirmation where base_entity_id = '" + baseEntityID + "'";

        DataMap<Date> dataMap = cursor -> getCursorValueAsDate(cursor, "amba_test_date", getNativeFormsDateFormat());
    ...
    public static Date getAmbaFollowUpVisitDate(String baseEntityID) {
        String sql = "SELECT eventDate FROM event where eventType ='Amba Follow-up Visit' AND baseEntityId ='" + baseEntityID + "'";
    ...
    public static void closeAmbaMemberFromRegister(String baseEntityID) {
        String sql = "update ec_amba_confirmation set is_closed = 1 where base_entity_id = '" + baseEntityID + "'";
        updateDB(sql);
    }

    public static boolean isRegisteredForAmba(String baseEntityID) {
        String sql = "SELECT count(p.base_entity_id) count FROM ec_amba_confirmation p " +
                "WHERE p.base_entity_id = '" + baseEntityID + "' AND p.is_closed = 0 AND p.amba  = 1 " +
    ...
    public static Integer getAmbaFamilyMembersCount(String familyBaseEntityId) {
        String sql = "SELECT count(emc.base_entity_id) count FROM ec_amba_confirmation emc " +
                ...
                "AND emc.is_closed = 0 AND emc.amba = 1";
    ...
    public static MemberObject getMember(String baseEntityID) {
        String sql = "select m.base_entity_id , m.unique_id , m.relational_id , m.dob , m.first_name , m.middle_name , m.last_name , m.gender , m.phone_number , m.other_phone_number , f.first_name family_name ,f.primary_caregiver , f.family_head , f.village_town ,fh.first_name family_head_first_name , fh.middle_name family_head_middle_name , fh.last_name family_head_last_name, fh.phone_number family_head_phone_number , ancr.is_closed anc_is_closed, pncr.is_closed pnc_is_closed, pcg.first_name pcg_first_name , pcg.last_name pcg_last_name , pcg.middle_name pcg_middle_name , pcg.phone_number  pcg_phone_number , mr.* from ec_family_member m inner join ec_family f on m.relational_id = f.base_entity_id inner join ec_amba_confirmation mr on mr.base_entity_id = m.base_entity_id left join ec_family_member fh on fh.base_entity_id = f.family_head left join ec_family_member pcg on pcg.base_entity_id = f.primary_caregiver left join ec_anc_register ancr on ancr.base_entity_id = m.base_entity_id left join ec_pregnancy_outcome pncr on pncr.base_entity_id = m.base_entity_id where m.base_entity_id ='" + baseEntityID + "' ";
        ...
        DataMap<MemberObject> dataMap = cursor -> {
            ...
            memberObject.setAmbaTestDate(getCursorValueAsDate(cursor, "amba_test_date", df));
