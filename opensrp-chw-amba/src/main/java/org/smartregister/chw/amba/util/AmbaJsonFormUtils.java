...
import org.smartregister.chw.amba.AmbaLibrary;
import org.smartregister.clientandeventmodel.Event;
import org.smartregister.domain.tag.FormTag;
import org.smartregister.repository.AllSharedPreferences;
import org.smartregister.util.FormUtils;

import static org.smartregister.chw.amba.util.Constants.ENCOUNTER_TYPE;
import static org.smartregister.chw.amba.util.Constants.STEP_ONE;
import static org.smartregister.chw.amba.util.Constants.STEP_TWO;

public class AmbaJsonFormUtils extends org.smartregister.util.JsonFormUtils {
    public static final String METADATA = "metadata";

    public static Triple<Boolean, JSONObject, JSONArray> validateParameters(String jsonString) {

        JSONObject jsonForm = toJSONObject(jsonString);
        JSONArray fields = ambaFormFields(jsonForm);
    ...
    public static JSONArray ambaFormFields(JSONObject jsonForm) {
        try {
    ...
    public static Event processJsonForm(AllSharedPreferences allSharedPreferences, String
        ...
        if (Constants.EVENT_TYPE.AMBA_CONFIRMATION.equals(encounter_type)) {
            encounter_type = Constants.TABLES.AMBA_CONFIRMATION;
        } else if (Constants.EVENT_TYPE.AMBA_FOLLOW_UP_VISIT.equals(encounter_type)) {
            encounter_type = Constants.TABLES.AMBA_FOLLOW_UP;
        }
    ...
    protected static FormTag formTag(AllSharedPreferences allSharedPreferences) {
        ...
        formTag.appVersion = AmbaLibrary.getInstance().getApplicationVersion();
        formTag.databaseVersion = AmbaLibrary.getInstance().getDatabaseVersion();
        return formTag;
    }
    ...
    public static void tagEvent(AllSharedPreferences allSharedPreferences, Event event) {
        ...
        event.setClientApplicationVersion(AmbaLibrary.getInstance().getApplicationVersion());
        event.setClientDatabaseVersion(AmbaLibrary.getInstance().getDatabaseVersion());
    }
    ...
    public static JSONObject getFormAsJson(String formName) throws Exception {
        return FormUtils.getInstance(AmbaLibrary.getInstance().context().applicationContext()).getFormJson(formName);
    }
